...
namespace Ookii.Dialogs
{
    ...
    public partial class ProgressDialog : Component
    {
        ...
        public ProgressDialog(IContainer container)
        {
            ...
            if( !Natives.IsWindowsVistaOrLater )
                Animation = AnimationResource.GetShellAnimation(Ookii.Dialogs.ShellAnimation.FlyingPapers);
        }
        ...
        /// </summary>
        /// <value>
        /// One of the values of <see cref="Ookii.Dialogs.ProgressBarStyle"/>. 
        /// The default value is <see cref="Ookii.Dialogs.ProgressBarStyle.ProgressBar"/>.
        /// </value>
        /// <remarks>
        /// <note>
        ...
        ///   progress bar will be hidden completely if this property is <see cref="Ookii.Dialogs.ProgressBarStyle.MarqueeProgressBar"/>.
        /// </note>
        /// <para>
        ///   When this property is set to <see cref="Ookii.Dialogs.ProgressBarStyle.ProgressBar" />, use the <see cref="ReportProgress(int)"/> method to set
        ///   the value of the progress bar. When this property is set to <see cref="Ookii.Dialogs.ProgressBarStyle.MarqueeProgressBar"/>
        ...
        /// </para>
        /// <para>
        ///   This method has no effect is <see cref="ProgressBarStyle"/> is <see cref="Ookii.Dialogs.ProgressBarStyle.MarqueeProgressBar"/>
        ///   or <see cref="Ookii.Dialogs.ProgressBarStyle.None"/>.
        /// </para>
        /// </remarks>
        ...
        private void RunProgressDialog(IntPtr owner, object argument)
        {
            ...
            if( Animation != null )
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(System.Globalization.CultureInfo.CurrentCulture, ex.Message), ex);
                }
            ...
            Interop.ProgressDialogFlags flags = Ookii.Dialogs.Interop.ProgressDialogFlags.Normal;
            if( owner != IntPtr.Zero )
                flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.Modal;
            switch( ProgressBarStyle )
            {
            case ProgressBarStyle.None:
                flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.NoProgressBar;
                break;
            case ProgressBarStyle.MarqueeProgressBar:
                if( Natives.IsWindowsVistaOrLater )
                    flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.MarqueeProgress;
                else
                    flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.NoProgressBar; // Older than Vista doesn't support marquee.
                break;
            }
            if( ShowTimeRemaining )
                flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.AutoTime;
            if( !ShowCancelButton )
                flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.NoCancel;
            if( !MinimizeBox )
                flags |= Ookii.Dialogs.Interop.ProgressDialogFlags.NoMinimize;
